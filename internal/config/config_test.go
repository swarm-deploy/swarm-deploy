...
func TestLoadWithSyncPrunePolicy(t *testing.T) {
	dir := t.TempDir()
...
`)
	...
	cfg, err := Load(configPath)
	require.NoError(t, err, "load config")
	assert.True(t, cfg.Spec.Sync.Policy.Prune, "expected global sync policy prune=true")
	require.Len(t, cfg.Spec.Stacks, 1, "expected one stack")
	require.NotNil(t, cfg.Spec.Stacks[0].Sync.Policy.Prune, "expected stack prune override")
	assert.False(t, *cfg.Spec.Stacks[0].Sync.Policy.Prune, "unexpected stack prune override")
}

func TestLoadWithSyncPrunePolicyUsesNilStackOverrideWhenMissing(t *testing.T) {
	dir := t.TempDir()
...
`)
	...
	require.NoError(t, err, "load config")
	require.Len(t, cfg.Spec.Stacks, 1, "expected one stack")
	assert.Nil(t, cfg.Spec.Stacks[0].Sync.Policy.Prune, "stack prune override must be nil when not configured")
}

func TestLoadAppliesDefaultStackDeployArgsWithoutPrune(t *testing.T) {
	dir := t.TempDir()
...
`)
	...
	assert.Equal(
		t,
		[]string{"stack", "deploy", "--with-registry-auth"},
		cfg.Spec.Swarm.StackDeployArgs,
		"unexpected default stack deploy args",
	)
}
