...
type ConfigsMapper struct{}

func (m *ConfigsMapper) Map(service *compose.Service, live swarm.StackService) {
	if live.ServiceSpec.TaskTemplate.ContainerSpec == nil {
		return
	}

	service.Configs = m.mapConfigs(live.ServiceSpec.TaskTemplate.ContainerSpec.Configs)
}

func (m *ConfigsMapper) mapConfigs(rawRefs []*dockerswarm.ConfigReference) []compose.ObjectRef {
	if len(rawRefs) == 0 {
		return nil
	}
