...
var (
	rn3AllowedHeaders = map[string]string{
		"POST": "Content-Type,X-Swarm-Deploy-Secret",
	}
)
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	default:
		...
		case '/': // Prefix: "/api/v1/"
			...
			case 'w': // Prefix: "webhooks/git"

				if l := len("webhooks/git"); len(elem) >= l && elem[0:l] == "webhooks/git" {
					elem = elem[l:]
				} else {
					break
				}
				...
				if len(elem) == 0 {
					...
					switch r.Method {
					case "POST":
						s.handleReceiveGitWebhookRequest([0]string{}, elemIsEscaped, w, r)
					default:
						s.notAllowed(w, r, notAllowedParams{
							allowedMethods: "POST",
		...
		case '/': // Prefix: "/api/v1/"
			...
			case 'w': // Prefix: "webhooks/git"
				...
				if len(elem) == 0 {
					...
					switch method {
					case "POST":
						r.name = ReceiveGitWebhookOperation
						r.summary = ""
						r.operationID = "receiveGitWebhook"
						r.operationGroup = ""
						r.pathPattern = "/api/v1/webhooks/git"
						r.args = args
						r.count = 0
						return r, true
					default:
