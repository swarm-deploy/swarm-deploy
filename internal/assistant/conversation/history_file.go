...
// TokenUsage contains model token consumption accumulated for a chat.
type TokenUsage struct {
	// InputTokens is the number of prompt tokens consumed by chat completions.
	InputTokens int64 `json:"input_tokens"`
	// OutputTokens is the number of completion tokens generated by chat completions.
	OutputTokens int64 `json:"output_tokens"`
	// TotalTokens is the total number of tokens reported by the model provider.
...
// IsZero reports whether the provider returned no token usage.
...
type Chat struct {
	...
	// Usage contains aggregate token consumption for successful chat completions.
...
type ChatSummary struct {
	...
	// UpdatedAt is the timestamp of the latest persisted message.
	UpdatedAt time.Time `json:"updated_at"`
	// Usage contains aggregate token consumption for the chat.
	Usage *TokenUsage `json:"token_usage,omitempty"`
}
...
func (s *FileHistoryStorage) List() []ChatSummary {
	...
	result := make([]ChatSummary, len(s.summaries))
	for idx, summary := range s.summaries {
		result[idx] = summary
		result[idx].Usage = cloneTokenUsage(summary.Usage)
	}
	return result
}
...
func (s *FileHistoryStorage) Append(id string, turns ...Turn) error {
	return s.AppendWithUsage(id, TokenUsage{}, turns...)
}

// AppendWithUsage appends turns and accumulates model token usage atomically.
func (s *FileHistoryStorage) AppendWithUsage(id string, usage TokenUsage, turns ...Turn) error {
	...
	s.upsertSummaryLocked(ChatSummary{
		...
		Usage:     cloneTokenUsage(chat.Usage),
	})
